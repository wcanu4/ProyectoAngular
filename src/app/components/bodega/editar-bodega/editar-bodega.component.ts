...
export class EditarBodegaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (id) {
      this.bodegaService.getStoreById(id).subscribe(
        (response) => {
          this.bodega = response;
          this.isLoading = false;
        },
        (error) => {
          console.error('Error al obtener el Partida:', error);
          this.isLoading = false;
        }
      );
    } 
 ...
 guardarCambios() {
    ...
    this.bodegaService.editarBodega(this.bodega.id, this.bodega).subscribe(
      (response) => {
        // Manejar la respuesta exitosa del servidor, si es necesario
        console.log('Partida actualizado exitosamente:', response);
      ...
      (error) => {
        ...
        console.error('Error al actualizar el partida:', error);
      }
    );
  }
  cancelar() {
